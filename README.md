...
- **TL;DR** is written in **ASD-STE100 Simplified Technical English** (the aerospace controlled-language standard): short sentences, one idea per sentence, plain approved-style vocabulary, active voice. Covers every signal, ordered roughly by rank, ending with any caution/caveat item. Note: STE trades density for clarity — the same content runs longer in STE than in normal prose (~230 words normal vs. ~280 words STE for a 12-signal week), so word-count targets don't carry over from the old rule; length is whatever STE needs to stay unambiguous.
